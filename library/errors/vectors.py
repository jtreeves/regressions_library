...
def multitype_vector(vector):
    ...
    if not isinstance(vector, list) or isinstance(vector[0], list):
        raise TypeError('Argument must be a 1-dimensional list')
...
def allow_none_vector(vector, position):
    ...
    if len(vector) > 1 and not isinstance(vector[1], (int, float)):
        ...
        return f'{identifier.capitalize()} is a 1-dimensional list that only contains integers, floats, or None; if it contains a second element, then its second element is an integer or a float'
